...
// Note: DEBUG flag is defined in newtab.js (global scope)
// Debug logger for Unsplash API
const unsplashLog = {
  info: (...args) => DEBUG && console.info('[Unsplash API]', ...args),
  warn: (...args) => DEBUG && console.warn('[Unsplash API]', ...args),
  error: (...args) => console.error('[Unsplash API]', ...args)
};
...
class UnsplashAPI {
  ...
  async getRandomPhoto(options = {}) {
    ...
    try {
      ...
      unsplashLog.error('Error fetching random photo:', error);
      throw error;
    }
  }
   ...
   */
  async getMultipleRandomPhotos(count = 10, options = {}) {
    const params = new URLSearchParams({
      client_id: this.accessKey,
      count: Math.min(count, 30), // Unsplash API max is 30
    ...
    try {
      ...
      // API returns array when count > 1, single object when count = 1
      return Array.isArray(data) ? data : [data];
    } catch (error) {
      unsplashLog.error('Error fetching multiple random photos:', error);
      throw error;
    }
  }
  ...
  async searchPhotos(query, page = 1, perPage = 30) {
    ...
    try {
      ...
      unsplashLog.error('Error searching photos:', error);
      throw error;
    }
  }
  ...
  async getCollectionPhotos(collectionId, page = 1, perPage = 30) {
    ...
    try {
      ...
      unsplashLog.error('Error fetching collection photos:', error);
      throw error;
    }
  }
  ...
  async trackDownload(downloadLocation) {
    ...
    try {
      ...
      unsplashLog.error('Error tracking download:', error);
    }
  }
   ...
   * @param {string} appName - Optional application name for UTM parameters
   * @returns {Object} Formatted photo data
   */
  formatPhotoData(photo, appName = '') {
    // Add UTM parameters for Unsplash attribution if appName is provided
    const utmParams = appName ? `?utm_source=${encodeURIComponent(appName)}&utm_medium=referral` : '';
    ...
    return {
      ...
      user: {
        ...
        profile: photo.user.links.html + utmParams
      },
      links: {
        html: photo.links.html + utmParams,
